...
!***********************************************************************
      ...
      subroutine error_ns(u, v, r, u0, v0, r0,
     &                   err_norm, rel_err,rmsu,rmsu_rel,err_max,
     &                     nx, ny, mom, pert_vel_max)
      ...
      double precision err_u_norm(nx,ny)
      double precision err_v_norm(nx,ny)
      double precision err_norm(nx,ny)
      double precision rel_err(nx,ny)
...
c          err_u_norm(i,j) =   ( u(i,j) - u0(i,j) ) / sqrt(u0_mag_sq)
c          err_v_norm(i,j) =  ( v(i,j) - v0(i,j) ) / sqrt(u0_mag_sq) ! velocity
         endif

          err_mag_sq = err_u(i,j)**2 + err_v(i,j)**2
          err_mag = sqrt(err_mag_sq) 
          
          err_norm(i,j) = err_mag/pert_vel_max

          rel_err(i,j) = sqrt(err_mag_sq / u0_mag_sq) 

          rmsu = rmsu + err_norm(i,j)**2
      ...
      rmsu_rel = sqrt(rmsu / float(nx*ny))
