...
c ----------------------------------------------------------------------
c0  cont0
      parameter(nx_e_quarter = 300) !quarter size of Euler (must be even) 
      parameter(ny_e_quarter = 300) !quarter size of Euler (must be even) 
...
! error
      ...
      double precision err_norm_e  (nx_e, ny_e)
...
c eulers solution on NS domain only (for two way error tracking)
      double precision u_n_2w  (nx_n, ny_n)
      double precision v_n_2w  (nx_n, ny_n)
      double precision p_n_2w  (nx_n, ny_n)
      double precision r_n_2w  (nx_n, ny_n)
...
c error arays
      ...
      double precision rel_err_n  (nx_n, ny_n)
      double precision err_norm_n  (nx_n, ny_n)
...
c     gradient of lagrange multiplier
      ...
      logical output_vtk_files ! output vtk files 
      ...
      open(unit=56,file='debug.dat',status='unknown')
      open(unit=64,file='energy_e.dat',status='unknown')
      open(unit=99,file='trash.dat',status='unknown')
...
c                     make sure n_p =0 above for Neumann
      ...
      ke_flux = .false. ! option for neum ! TRASH

      conv  = .false. ! convective bc's TRASH !
...
c vortex strength (original: dlambda = 2.d0, deta = 1.d0, dgam = 1.4d0)
c (small: dlambda = 0.4688d0, deta = 3.d0, dgam = 1.4d0, need 6 acoustic substeps)
c vparam
...
c mean velocity for outputvtk_ns and outputvtk (subtracts it)
...
! and for "true" energy calculations in case of 2way coupling

      call init_euler(u0_e, v0_e, r0_e, x_e, xc_e, y_e, yc_e,
     &                      ix, iy, nx_e, ny_e, nb_e, vortex_center0,
     &                       umean, vmean, rhomean,
     &                       dlambda, deta, dgam)

! calc max pertutbation velocity in Euler Domain (send it to error calc)
! i.e. normalization constant
      pert_vel_max = 0.d0
      do j = 1,ny_e
        do i = 1,nx_e
          pert_vel = sqrt((u0_e(i,j)-umean)**2+(v0_e(i,j)-vmean)**2) 
          if (pert_vel .gt. pert_vel_max) then
              pert_vel_max = pert_vel
          endif
        enddo
      enddo

      write(*,*) 'pert_vel_max', pert_vel_max
!      stop
 
      call update_pressure(p_e, r_e, zero, nx_e, ny_e)
      call update_pressure(p0_e, r0_e, 0., nx_e, ny_e)
...
c      dt = min(2.*viscosity_n / u_max**2 , 0.5*dx_e**2 / viscosity_n)
...
c      dt = courant_e * dt
      ...
      itmid = int(itmax / 2) ! fot the vtk output in the middle of the run
...
c     force acoustic # of steps 
c      dt = 9.3196644920782844d-3 
c      itmax =  1 + int(tmax / dt)
      
      ns = 2 
      write(*,*) 'acoustic substeps', ns
...
c----------write run parameters into file params.dat--------------------
       ...
       write(50,*) 'n_p',n_p
       write(50,*) '# of acoustic substeps ns',ns
...
c-------------euler--"true"--solution------------------------------
        call update_euler(t, u0_e, v0_e, r0_e, p0_e,
     &                    rux0, ruy0, rvy0, rvx0, rrx0, rry0,
     &                    dtau, iacoustic, dx_e, dy_e, u_max,
     &                    ix, iy, nb_e, nx_e, ny_e, k)
...
c7---------debug: checking south vs north--------------------
c test      
c      do i = 1, ny_n
c      write(56,*) v_bc_ns(i,1)
c      write(56,*) u_bc_ew(i,1)
c      write(56,*) re_bc_ew(i,1)
c      write(56,*) re_bc_ns(i,1)
cc      write(56,*) v_bc_ew(i,1)
c      write(56,*) u_bc_ns(i,1)
c      enddo
c      if (k .eq. 300) then 
c      stop
c      endif
...
c------debug: calc divergence after NS step---------------
        ...
        if (two_way) then
          ...
          if (neum) then
            ...
            call overwrite_n_to_e(u_e, v_e, r_e,
     &                            u_n, v_n, density_n,
     &                            nx_n, ny_n, nx_e, ny_e,
     &                            nx_start, ny_start)
...
c                 START VTK
c------------------------------------------------------------------------
        output_vtk_files = .false.
c c4
        if (dt_vtk .ge. dt_vtk_out) then
          dt_vtk = 0.d0
          i_vtk_out = i_vtk_out + 1
       
c output vtk files only in the middle and the end of the run
      if(((t .ge. 4.9d0) .and. (t .le. 5.1d0)) .or. (t .gt. 9.9d0)) then
       output_vtk_files = .true.
      endif 
...
c EULER
c error
         call error(u_e,v_e,r_e,p_e,u0_e,v0_e,r0_e,p0_e,rmsu_e,rmsr_e,
     &           rmsu_rel_e,err_max_e,err_norm_e, err_u_e, err_v_e,
     &           err_rho_e,err_p_e, nx_e, ny_e, pert_vel_max)

        write(53,*) t,rmsu_e,rmsu_rel_e,err_max_e

cEULER vtk 
c outputvtk subtracting mean:
       if (output_vtk_files) then

         call outputvtk(u_e,v_e,r_e,p_e,xmin_e, ymin_e,dx_e,dy_e,
     &                  ix,iy,nx_e,ny_e,nb_e,
     &                 'output_e'//filename(i_vtk_out),
     &                  umean, vmean, rhomean, pmean )


         call outputvtk_err(err_u_e,err_v_e,err_norm_e,err_p_e,
     &                      xmin_e,ymin_e,
     &                      dx_e,dy_e,ix,iy,nx_e,ny_e,nb_e,
     &                      'output_e_err'//filename(i_vtk_out))
...
c FOR ERROR output 
c         call outputvtk(err_u_e,err_v_e,err_norm_e,p_e,
c     &                   xmin_e, ymin_e,dx_e,dy_e,
c     &                  ix,iy,nx_e,ny_e,nb_e,
c     &                 'output_e_orr'//filename(i_vtk_out),
c     &                  0.d0, 0.d0, 0.d0, pmean )
...
c     &                          dx_n, dy_n, nx_n, ny_n, n_p) !  
          ...
          call copy_e_to_n(u0_e, v0_e, r0_e,
     &                    u0_n, v0_n, r0_n,
     &                    nx_e, ny_e, nx_n, ny_n,
     &                    nx_start, ny_start)

c now copy Euler solution onto NS domain only(for two way)

      if (two_way) then
          call copy_e_to_n(u_e, v_e, r_e,
     &                    u_n_2w, v_n_2w, r_n_2w,
     &                    nx_e, ny_e, nx_n, ny_n,
     &                    nx_start, ny_start)
      endif 
...
c energy routines 
      ...
      if (two_way) then
        ...
        call calc_energy_n(energy_n, energy2d_n, u_n_2w, v_n_2w, r_n_2w,
     &                     eddy_ke_n,dx_n, dy_n, nx_n, ny_n)
      else
     ...
     &                     eddy_ke_n0,dx_n, dy_n, nx_n, ny_n)

        call calc_energy_e(energy_e,eddy_ke_e,u_e,v_e,r_e,
     &                     dx_e,dy_e,nx_e,ny_e)

        call calc_energy_e(energy_e0,eddy_ke_e0,u0_e,v0_e,r0_e,
     &                     dx_e,dy_e,nx_e,ny_e)
     ...
     &                 nx_n, ny_n)
          ...
          write(*,*)'energy', i_vtk_out, t, energy_n, energy_n0 


          write(54,*) i_vtk_out,t,energy_n,energy_n0,rmse,
     &                eddy_ke_n, eddy_ke_n0 


          write(64,*) i_vtk_out,t,energy_e,energy_e0,
     &                eddy_ke_e, eddy_ke_e0
...
c    &                      umean,vmean,pmean,ns_vtk_option)
      ...
      if (two_way) then
          ...
          call error_ns(u_n_2w, v_n_2w, r_n_2w, u0_n, v0_n, r0_n,
     &              err_norm_n, rel_err_n, rmsu_n, rmsu_rel_n,err_max_n,
     &              nx_n, ny_n, .false., pert_vel_max)
      else
          call error_ns(u_n, v_n, r_n, u0_n, v0_n, r0_n,
     &              err_norm_n, rel_err_n, rmsu_n, rmsu_rel_n,err_max_n,
     &              nx_n, ny_n, .false., pert_vel_max)
      endif

c ITS NORMALIZED ERROR INSTEAD OF p_n (thats why its 0.d0 istead of pmean)

       if (output_vtk_files) then

          call outputvtk_ns(u_n, v_n, err_norm_n, div_n,
     &                      xmin_n, ymin_n, dx_n, dy_n, nx_n, ny_n,
     &                      'output_ns'//filename(k),
     &                      umean,vmean,0.d0,ns_vtk_option)
       endif

c ITS LAGRANGE MULT FIELD OF p_n (thats why its 0.d0 istead of pmean)
c
c
c          call outputvtk_ns(u_n, v_n, lag_mult, div_n,
c     &                      xmin_n, ymin_n, dx_n, dy_n, nx_n, ny_n,
c     &                      'output_ns'//filename(i_vtk_out),
c     &                      umean,vmean,0.d0,ns_vtk_option)
c
c
c          call  outputvtk_bound(u_bc_ew, u_bc_ns,
c     &                 v_bc_ew, v_bc_ns,
c     &                 xmin_n, ymin_n, xmax_n, ymax_n,
c     &                 umean, vmean,
c     &                 dx_n,dy_n,nx_n,ny_n,
c     &                 'output_bound'//filename(i_vtk_out))



c      call outputvtk_ns(err_u_n, err_v_n, rel_err_n, div_n,
c     &                      xmin_n, ymin_n, dx_n, dy_n, nx_n, ny_n,
c     &                      'output_ns_err'//filename(i_vtk_out),
c     &                      0.d0, 0.d0, 0.d0, 1)
